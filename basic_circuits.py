# Given an edge list, find the basic circuits.
# Bruce Wernick
# 16 May 2025
...
def prim_mst(graph, start_node=0):
  ...
  edges = [(cost, start_node, to) for to, cost in graph.adj[start_node]]
  heapq.heapify(edges)
  while edges:
    ...
    if v not in visited:
      visited.add(v)
      mst.append((u, v, cost))
      for np, c in graph.cost_neighbors(v):
        if np not in visited:
          heapq.heappush(edges, (c, v, np))
  return mst

def to_adj(edges):
  """ Convert edge list [(u,v,c)...] to adjacency dictionary.
  """
...
for e in graph.edges:
  u, v, c = e
  if (u, v, c) not in mst and (v, u, c) not in mst:
    basic.append(e)
print(f"basic: {basic}")
...
adj = to_adj(mst)
g.adj = adj
for i, (u, v, c) in enumerate(basic):
  path = search(g, v, u)
  print(f"circ[{i}]: {path}")
